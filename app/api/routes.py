...
def create_app(service=None, settings=None):
    app=Flask(__name__); settings=settings or Settings.from_env()
    if service is None:
        db=create_client(settings.project_id,settings.firestore_database); creds=CredentialProvider(FirestoreTokenStore(db)); service=AutoEmailService(settings,FirestoreRunRepository(db),GmailDrafts(GmailClientFactory(creds)),TelegramNotifier(settings.telegram_bot_token,settings.telegram_chat_id,settings.telegram_timeout_seconds))
    @app.errorhandler(AutoEmailError)
    def known_error(error): return jsonify({'status':'error','error':{'code':error.code,'message':error.safe_message}}),error.http_status
    @app.errorhandler(Exception)
    def unknown_error(error): log.exception('unhandled_request_error',extra={'event':'unhandled_request_error'}); return jsonify({'status':'error','error':{'code':'internal_error','message':'The request could not be completed.'}}),500
    @app.get('/healthz')
    def healthz(): return jsonify({'status':'live'}),200
    @app.get('/readyz')
    def readyz():
    ...
    def execute(store_code, correction=False):
        payload=request.get_json(silent=True) or {}; date_value=None
        if correction:
            try: date_value=date.fromisoformat(payload['business_date'])
            except (KeyError,TypeError,ValueError): return jsonify({'status':'error','error':{'code':'invalid_business_date','message':'business_date must be YYYY-MM-DD.'}}),400
            if not isinstance(payload.get('correction_request_id'),str) or not payload['correction_request_id'].strip(): return jsonify({'status':'error','error':{'code':'correction_request_id_required','message':'correction_request_id is required.'}}),400
